...
# データファイルリスト
made_files = [f"made{i}.csv" for i in range(1, 7)]
moji_files = [f"moji{i}.csv" for i in range(1, 7)]
...
# 7. パラメータ範囲ループで精度評価
th_range = list(range(200, 1001, 100))
...
# 各ファイルごと・各閾値ごとに TP, FP, TN, FN を記録
# 例: TP_list[freq_idx][th_idx][file_idx]
TP_list = np.zeros((len(freq_range), len(th_range), 6), dtype=int)
FP_list = np.zeros_like(TP_list)
TN_list = np.zeros_like(TP_list)
FN_list = np.zeros_like(TP_list)





def process_one_pair(made_file, moji_file, file_idx):
    # ==== データ読み込み ====
    df = pd.read_csv(made_file)
    # 2. 文字ごとに「line」列を自動で付与（行ごとの位置調整、ここは元コードそのまま）
    moji_df = pd.read_csv(moji_file)
    ...
    for j, th in enumerate(th_range):
        ...
        for i, freq_th in enumerate(freq_range):
            ...
            TP_list[i, j, file_idx] = TP
            FP_list[i, j, file_idx] = FP
            TN_list[i, j, file_idx] = TN
            FN_list[i, j, file_idx] = FN
...
# ==== 全体ループ ====
for i in range(6):
    process_one_pair(made_files[i], moji_files[i], i)



# 全ファイル分終わったら、閾値ごとに合計して指標を計算
F1_mat = np.zeros((len(freq_range), len(th_range)))
PREC_mat = np.zeros_like(F1_mat)
RECALL_mat = np.zeros_like(F1_mat)
ACC_mat = np.zeros_like(F1_mat)

for freq_i in range(len(freq_range)):
    for th_j in range(len(th_range)):
        TP_total = TP_list[freq_i, th_j, :].sum()
        FP_total = FP_list[freq_i, th_j, :].sum()
        TN_total = TN_list[freq_i, th_j, :].sum()
        FN_total = FN_list[freq_i, th_j, :].sum()
        ...
        F1_mat[freq_i, th_j] = f1
        PREC_mat[freq_i, th_j] = precision
        RECALL_mat[freq_i, th_j] = recall
        ACC_mat[freq_i, th_j] = accuracy
...
# 最適な閾値を選ぶ
best_idx = np.unravel_index(F1_mat.argmax(), F1_mat.shape)
print(f"全体で最大F1={F1_mat[best_idx]:.3f}（秒数閾値: {th_range[best_idx[1]]}, 頻度閾値: {freq_range[best_idx[0]]}）")
print(f"Precision={PREC_mat[best_idx]:.3f}, Recall={RECALL_mat[best_idx]:.3f}, Accuracy={ACC_mat[best_idx]:.3f}")